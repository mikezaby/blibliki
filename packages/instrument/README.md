...
- `src/recipes/`: starting documents a new instrument can be created from, one file per recipe, listed in `instrumentRecipes`. A recipe is `{ id, title, description, document }`. To add one, tune an instrument in the app, save it, copy its document into a new file and add it to `src/recipes/index.ts`. `recipeDocument.ts` and `stepLanes.ts` are the shorthand the first recipes are written in.

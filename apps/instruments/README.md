...
The Blibliki logo is `Logo` from `@blibliki/ui`, shared with grid. The picker
header shows the mark, and the console carries its own badge (see the
[console's README](../../packages/instrument/README.md#the-react-entry-point)).

`public/favicon.svg` is the same path as the component, with its own fills,
dark on a light browser and light on a dark one, since a favicon cannot
inherit a colour. `public/favicon.ico` (16, 32 and 48px) and
`public/apple-touch-icon.png` (180px) are renders of that path in `#fafafa` on
`#09090b`, for browsers that do not take an SVG favicon and for the iOS home
screen. `apps/grid/public` holds the same three files. None of them are
generated by the build. If the path changes, render them again for both apps.
